...
export default function BillDetailsFacturation({bill}){
    ...
    let firstDay = dateFormat(new Date(date.getFullYear(), date.getMonth(), 1), "dd.mm.yyyy");
    let lastDay = dateFormat(new Date(date.getFullYear(), date.getMonth() + 1, 0), "dd.mm.yyyy");


    const link = "customer_sessions?page=1&itemsPerPage=100&customer.id="+bill.customer.id+"&session.day_at%5Bbefore%5D="+lastDay+"&session.day_at%5Bafter%5D="+firstDay;
  ...
  useEffect(() => {
    ...
    if(!customerSessions){
        return <div className="column column--lg_5"><Loading /></div>
    }
    return(
                        ...
                        </tr>
                        {
                            customerSessions["hydra:member"].map((cs) => (
                                    <tr key={cs.id}>
                                        ...
                                        <td>{user.association.bankInformation.currency} {cs.session.session_type.price_of}</td>
                                    </tr>
                            ))
                        }
                        {<ItemList items={bill.items} currency={user.association.bankInformation.currency} />}
                        <tr>
                            <th colSpan="2">Total</th>
                            <th>{user.association.bankInformation.currency} {bill.amount}</th>
                        </tr>
                    </tbody>
                </table>
      </div>
