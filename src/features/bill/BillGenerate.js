...
export default function BillGenerate() {
    ...
    apiBni
      ...
      .then((response) => {
        if (response.status === 201) {
            dispatch(setAlert({ "color":"success", "message":"Les factures on bien étées crées"}));
  ...
  const itemSchema = Yup.object().shape({
    ...
    from_at: Yup.string().matches(/^(0[1-9]|[12][0-9]|3[01]).(0[1-9]|1[012]).(19|20)\d\d$/, 'Format dd.mm.yyyy').required('Date Requise'),
    ...
    return (
      <>
          <div className="container-fluid p-5 d-flex flex-column justify-content-center align-items-center">
            ...
            <Formik
              onSubmit={submit}
              initialValues={{ billing_month:month+1, billing_year:year, itemList: [], from_at: "" }}
              ...
              {({
                ...
                <form onSubmit={handleSubmit} className="bg-white border p-5 d-flex flex-column">
                  ...
                  <div className="form-group">
                    ...
                    >
                      { monthFR.map((m) => {                        
                       return <option value={m[0]} key={m[0]+m[1]}>{m[1]}</option>
                      })}
                     ...
                     {itemsOptions.map((i) => {
                       return (
                        ...
                        <Field
                          ...
                          onBlur={handleBlur}
                        />
                        <label className="form-check-label" htmlFor={i.value+i.name}>{i.name}</label>
                        </div>
                       )
                     })}                                                  
